...
## Ускорение загрузки (сделано)

- Убраны тяжёлые скрипты: Yandex Ads, fpm, Clearfy, Swiper
- Локальные скрипты — с `defer`
- Обработчик «Скачать» заменён на простой `window.open` вместо Ya.Context.AdvManager

**Дополнительно (опционально):**

1. **Картинки** — уже `loading="lazy"` в post-cards; остальные посты — при желании можно прогнать скрипт
2. **CDN** — статику (css, js, uploads) можно раздавать через Cloudflare/другой CDN
3. **Сжатие** — включить gzip/brotli на сервере
4. **Кэш** — в .htaccess добавить Cache-Control для `wp-content/`
